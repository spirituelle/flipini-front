...
/**
 * Decide where catalog is in cart.
 * @param { Array } cartItems 
 * @param { Object } catalog 
 */
export const isInCart = ( cartItems, catalog ) => {
    return cartItems.find( item => item.id == catalog.id ) ? true : false;
}

/**
 * Confirm if catalog could be added to cart.
 * @param { Array } cartItems 
 * @param { Object } catalog 
 * @param { Number } qty 
 */
export const canAddToCart = ( cartItems, catalog, qty ) => {
    let find = cartItems.find( item => item.id == catalog.id );
    if ( find ) {
        if ( catalog.stock == 0 || ( catalog.stock < ( find.qty + qty ) ) ) return false;
        else return true;
    } else {
        if ( catalog.stock == 0 || ( catalog.stock < qty ) ) return false;
        else return true;
    }
}
 ...
 * @param {Object} catalog 
 */
export const isInWishlist = ( wishlist, catalog ) => {
    return catalog && wishlist.findIndex( item => item.slug == catalog.slug ) > -1;
}
 ...
 * @param {Object} catalog 
 */
export const isInCompare = ( compare, catalog ) => {
    return catalog && compare.findIndex( item => item.slug == catalog.slug ) > -1;
}
